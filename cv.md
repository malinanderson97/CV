# Malin Anderson

Brighton, England | 07562248838 | malinanderson97@gmail.com

## Professional Summary
Second-year Politics, Philosophy & Economics (PPE) student and Cultural Policy Researcher combining rigorous academic methodologies with extensive experience in data analytics, market research, and strategic communications. Adept at translating complex cultural and market data into actionable insights and thought leadership content. Seeking to leverage quantitative analysis, structural analysis, and executive writing skills to support policy, research, or operational projects.

## Education
**Politics, Philosophy & Economics Student | Sussex University**
September 2024 - Present

* Full-time student at a Russell Group University, currently averaging a First-Class Honours.
* **Key Methodologies:** Quantitative data analysis, economic modeling, structural institutional analysis, and formal logic.
* **Selected First-Class Module Results:** Macroeconomics 1 (94%), Introduction to Mathematics for Finance and Economics (87%), Reason and Argument (80%), Microeconomics 1 (78%), Microeconomics 2 (73%), and European Politics (72%).

## Relevant Professional Experience

**Creator & Developer | Economo**
June 2026 - Present

* Developed an interactive economic simulation tool translating Blanchard’s IS-MP-PC textbook models into a live, parameter-driven computational engine for intermediate learners.
* Modeled complex economic relationships computationally, calculating derived multipliers and reduced-form responses across goods markets, monetary policy, and exchange rate dynamics.
* Implemented mathematical assertions and testing scripts to rigorously verify that the tool's computational outputs reconciled perfectly with theoretical textbook derivations.
* Designed data-driven visualizations to map complex theoretical concepts into intuitive, interactive learning modules.

**Founder & Operator | E-commerce Reselling Business**
October 2024 - Present

* Manage end-to-end e-commerce operations, including inventory sourcing, dynamic pricing, and customer relations.
* Apply applied microeconomic principles to analyze market trends, optimize profit margins, and identify high-demand, low-supply retail niches.

**Digital Marketing Strategist | The Butcher Corporation**
March 2023 - Present

* Provide regular performance reports and extract actionable insights using Google Analytics.
* Collaborate with content teams to research and produce thought leadership content.
* Manage strategic campaigns, successfully driving a 30% increase in website traffic and generating 25% more leads.

**Event Organizer | Blitz Events LTD**
April 2024 - Present

* Sole organizer of a nighttime event series combining Contemporary Art and Dance Music.
* Manage end-to-end logistics, operational frameworks, and strategic planning.

**Social Media Manager | Alma Art**
September 2022 - December 2022

* Conducted in-depth market research and devised a comprehensive strategic plan for a Web3 digital collection.
* Authored website copy, launched a blog series, and created interactive content to successfully drive audience engagement.
* Grew digital audience from 0 to 28K in under a month and secured the #4 trending spot on popular marketplaces.

**Co-Founder | Dexter Inspire & Dexter Studios**
March 2018 - October 2020

* Managed multimedia resources, technical workflows, budgeting, and end-to-end project execution.
* Utilized innovative software applications to optimize operational efficiency, reducing programming time from one month to under a week.
* Delivered projects for high-profile clients including British Airways and RHS Hampton Court Garden Festival.

## Core Competencies

* **Research & Analysis:** In-depth Market Research, Data & Google Analytics, Macroeconomic Modeling, IS-LM/IS-MP-PC Frameworks, Comparative Statics.
* **Technical Skills:** JavaScript (ES6+), HTML5, CSS3, Node.js, Interactive Data Visualization.
* **Writing & Communications:** Executive Copywriting, Thought Leadership Production.
* **Project Management:** Budgeting, Technical Workflows, E-commerce Operations.
